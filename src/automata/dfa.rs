...
impl Dfa {
    ...
    pub fn minimize(&mut self) -> Result<(), &'static str> {
        ...
        for (index, set) in old_eqvl.iter().enumerate() {
            // save the state index ad it's new name in the dfa
            lookup_table.insert(set, index as u32);
            dfa.add_state();

            // if the set contains accept states, the index is an accept state
            if set.iter().any(|&state| self.is_accept_state(state)) {
                dfa.add_accept_state(index as u32);
            }
        }
        dfa.remove_state(); // remove excess
    ...
    fn increment_states(&mut self, increment: u32) {
        let mut lookup_table: HashMap<(u32, char), u32> = HashMap::new();

        self.states.iter_mut().for_each(|e| *e += increment);
        self.accept_states.iter_mut().for_each(|e| *e += increment);

        self.transition_fn.iter().for_each(|((source, symbol), target)| {
            lookup_table.insert((source + increment, *symbol), target + increment);
        });
    ...
    pub fn to_regex(&self) -> String {
        regex::get_regex(self).to_string()
    }
}
